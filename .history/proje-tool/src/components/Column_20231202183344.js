import _ from 'lodash';
import React, { useMemo } from 'react'
import { shuffle} from 'lodash';
import { useDispatch, useSelector } from 'react-redux'
import boardsSlice from '../redux/boardsSlice'
import Task from './Task'

const colors = [
    "bg-red-500",
    "bg-orange-500",
    "bg-yellow-500",
    "bg-green-500",
    "bg-teal-500",
    "bg-blue-500",
    "bg-indigo-500",
    "bg-purple-500",
    "bg-pink-500",
];

<<<<<<<<<<<<<  ✨ Codeium AI Suggestion  >>>>>>>>>>>>>>
-function Column({ col }) {
-    const dispatch = useDispatch();
-
-    const color = useMemo(() => _.sample(colors), [colIndex]);
-
-    const handleOnDrop = useCallback((e) => {
-        try {
-            const { prevColIndex, taskIndex } = JSON.parse(
-                e.dataTransfer.getData("text")
-            );
-
-            if (colIndex !== prevColIndex) {
-                dispatch(
-                    boardsSlice.actions.moveTask({
-                        prevColIndex,
-                        newColIndex: colIndex,
-                        taskIndex,
-                    })
-                );
-            }
-        } catch (error) {
-            
-        }
-    }, [colIndex, dispatch]);
-
-    const handleOnDragOver = useCallback((e) => {
-        e.preventDefault();
-    }, []);
-
-    const renderTasks = (tasks, colIndex) => {
-        return tasks.map((task) => (
-            <Task
-                key={task.id}
-                taskIndex={task.index}
-                colIndex={colIndex}
-                task={task}
-            />
-        ));
-    };
-
-    return (
-        <div
-            onDrop={handleOnDrop}
-            onDragOver={handleOnDragOver}
-            className="scrollbar-hide mx-5 pt-[90px] min-w-[280px]"
-        >
-            <p className="font-semibold flex items-center gap-2 tracking-widest md:tracking-[.2em] text-[#828fa3]">
-                <div className={`rounded-full w-4 h-4 ${color}`} />
-                {col.name} ({col.tasks.length})
-            </p>
-
-            {renderTasks(col.tasks, colIndex)}
-        </div>
-    );
+/**
+ * Renders a column component.
+ * @param {Object} props - The properties object.
+ * @param {Object} props.col - The column object.
+ * @returns {JSX.Element} - The rendered column component.
+ */
+function Column({ col }: { col: { name: string, tasks: Array<Object> } }): JSX.Element {
+    const dispatch = useDispatch();
+
+    const color = useMemo(() => _.sample(colors), [colIndex]);
+
+    const handleOnDrop = useCallback((e: React.DragEvent) => {
+        try {
+            const { prevColIndex, taskIndex } = JSON.parse(
+                e.dataTransfer.getData("text")
+            );
+
+            if (colIndex !== prevColIndex) {
+                dispatch(
+                    boardsSlice.actions.moveTask({
+                        prevColIndex,
+                        newColIndex: colIndex,
+                        taskIndex,
+                    })
+                );
+            }
+        } catch (error) {
+            
+        }
+    }, [colIndex, dispatch]);
+
+    const handleOnDragOver = useCallback((e: React.DragEvent) => {
+        e.preventDefault();
+    }, []);
+
+    const renderTasks = (tasks: Array<Object>, colIndex: number) => {
+        return tasks.map((task) => (
+            <Task
+                key={task.id}
+                taskIndex={task.index}
+                colIndex={colIndex}
+                task={task}
+            />
+        ));
+    };
+
+    return (
+        <div
+            onDrop={handleOnDrop}
+            onDragOver={handleOnDragOver}
+            className="scrollbar-hide mx-5 pt-[90px] min-w-[280px]"
+        >
+            <p className="font-semibold flex items-center gap-2 tracking-widest md:tracking-[.2em] text-[#828fa3]">
+                <div className={`rounded-full w-4 h-4 ${color}`} />
+                {col.name} ({col.tasks.length})
+            </p>
+
+            {renderTasks(col.tasks, colIndex)}
+        </div>
+    );
}
<<<<<  bot-a89a47e3-952d-45c6-a5f7-7a0e6c8448b8  >>>>>

export default Column