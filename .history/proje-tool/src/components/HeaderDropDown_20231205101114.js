 ...
 * 
 * @param {boolean} checked - Indicates whether the dark mode is checked or not.
 * @returns {void}
 */
...
useEffect(() => {
    localStorage.setItem('darkMode', JSON.stringify(darkSide));
}, [darkSide]);

useEffect(() => {
    const darkMode = localStorage.getItem('darkMode');
    if (darkMode) {
        setDarkSide(JSON.parse(darkMode));
    }
})
            ...
            <div className="dropdown-board">
                ...
                <div className="flex items-baseline space-x-2 px-5 py-4 text-[#635fc7]">
                    ...
                    <div className="mx-2 p-4 space-x-2 bg-slate-100 dark:bg-[#20212c] flex justify-center items-center rounded-lg">
                        ...
                        <img src="./assets/icon-dark-theme.svg" alt="moon indicating dark mode"/>
                    </div>  
            </div>
        </div>
