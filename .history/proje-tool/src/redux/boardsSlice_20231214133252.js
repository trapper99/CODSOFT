...
const boardsSlice = createSlice({
    ...
    reducers: {
        ...
        addTask: (state, action) => {
            const { title, status, description, subtasks, newColIndex, isCompleted} = action.payload;
            const task = { title, description, subtasks, status, isCompleted };
            const board = state.find((board) => board.isActive);
        ...
        editTask: (state, action) => {
            ...
            task.title = title;
            task.status = status;
            task.isCompleted = isCompleted;isCompleted
        ...
        setSubtaskCompleted: (state, action) => {
            const payload = action.payload;
            const board = state.find((board) => board.isActive);
            const col = board.columns.find((_task, i) => i === payload.colIndex);
            const task = col.tasks.find((_task, i) => i === payload.taskIndex);
            const subtask = task.subtasks.find((_task, i) => i === payload.index);
