...
function AddEditTaskModal({
  ...
  const board = useSelector((state) => state.boards).find((board) => board.isActi)
...
function CurrentStatus({ status, onChangeStatus, onSubmit, isValid, type }) {
  return (
    <div className="mt-8 flex flex-col space-y-3">
      ...
      >
        {columns.map((column, index) => (
          <option key={column.id} value={column}>
            {column.name}
          </option>
        ))}
      </select>
